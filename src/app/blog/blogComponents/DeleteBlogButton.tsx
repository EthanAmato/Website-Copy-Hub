    ...
    return (
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        <DialogTrigger asChild>
          <Button className="bg-red-400 text-black hover:bg-red-700 hover:text-white" onClick={() => setIsOpen(true)}>
            Delete Post
          </Button>
        </DialogTrigger>
        ...
        <DialogContent className="fixed left-1/2 top-1/2 z-50 -translate-x-1/2 -translate-y-1/2 rounded-lg bg-white p-6 shadow-lg sm:max-w-[425px]">
          ...
          <DialogFooter>
            <Button onClick={handleDelete} variant="destructive" className="text-black" color="red">
              Delete
            </Button>
            <Button variant="outline" onClick={() => setIsOpen(false)}>Cancel</Button> {/* Directly close the dialog on cancel */}
          </DialogFooter>
        </DialogContent>
      </Dialog>
    );
